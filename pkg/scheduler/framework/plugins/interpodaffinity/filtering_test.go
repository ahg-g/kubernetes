...
func TestRequiredAffinitySingleNode(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			p := plugintesting.SetupPlugin(ctx, t, New, &config.InterPodAffinityArgs{}, snapshot, namespaces)
...
func TestRequiredAffinityMultipleNodes(t *testing.T) {
	...
	for indexTest, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			p := plugintesting.SetupPlugin(ctx, t, New, &config.InterPodAffinityArgs{}, snapshot,
				[]runtime.Object{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			getState := func(pods []*v1.Pod) (*InterPodAffinity, *framework.CycleState, *preFilterState, *cache.Snapshot) {
				...
				p := plugintesting.SetupPlugin(ctx, t, New, &config.InterPodAffinityArgs{}, snapshot, nil)
				cycleState := framework.NewCycleState()
